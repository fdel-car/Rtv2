...
float intersect_sphere(t_obj *obj, t_data ray)
{
	...
	if (!strcmp("sphere_oculus", obj->name) && !g_env.oculus)
		return (-1);
	a = SQ(ray.dir.x) + SQ(ray.dir.y) + SQ(ray.dir.z);
	b = 2.0 * (ray.dir.x * (ray.orig.x - obj->pos.x) + ray.dir.y * (ray.orig.y - obj->pos.y) + ray.dir.z * (ray.orig.z - obj->pos.z));
	c = (SQ(ray.orig.x - obj->pos.x) + SQ(ray.orig.y - obj->pos.y) +
		 SQ(ray.orig.z - obj->pos.z)) -
		SQ(obj->rayon);
...
float intersect_plane(t_obj *obj, t_data ray)
{
	...
	if (solut > EPSILON)
	{
		...
		if (SQ(hit.x - obj->pos.x) + SQ(hit.y - obj->pos.y) + SQ(hit.z - obj->pos.z) > SQ(obj->sphere_cut) && obj->sphere_cut != 0)
			return (-1);
...
float intersect_cone(t_obj *obj, t_data ray)
{
	...
	b = 2.0 * (vec_dotp(ray.dir, tmp) - ((1.0 + SQ(obj->alpha)) * vec_dotp(ray.dir, obj->dir) * vec_dotp(tmp, obj->dir)));
	...
	if (!quadratic_root(a, b, c, r))
		return (-1);
	return (texture_cutr(cut_basics(&ray, obj, vec_is_null(obj->cut) ? obj->dir : obj->cut, r), r, obj, ray));
}
