...
{
	if (strcmp(data->desc, "shiny") == 0)
	{
		*ret_f = atof(gtk_entry_get_text(entry));
		((t_obj *)data->obj)->mater.shiny = *ret_f;
	}
	if (strcmp(data->desc, "refl") == 0)
	{
		*ret_f = atof(gtk_entry_get_text(entry));
		((t_obj *)data->obj)->mater.int_refl = *ret_f;
	}
	if (strcmp(data->desc, "trans") == 0)
	{
		*ret_f = atof(gtk_entry_get_text(entry));
		((t_obj *)data->obj)->mater.int_trans = *ret_f;
	}
	if (strcmp(data->desc, "colr") == 0)
	{
		*ret_i = atoi(gtk_entry_get_text(entry));
...
void save_entry_material_object3(t_gtkdata *data, char **ret_s,
								 int *ret_i, GtkEntry *entry)
{
	if (strcmp(data->desc, "colg") == 0)
	{
		*ret_i = atoi(gtk_entry_get_text(entry));
		((t_obj *)data->obj)->mater.color.g = *ret_i;
	}
	if (strcmp(data->desc, "colb") == 0)
	{
		*ret_i = atoi(gtk_entry_get_text(entry));
		((t_obj *)data->obj)->mater.color.b = *ret_i;
	}
	if (strcmp(data->desc, "text") == 0)
	{
