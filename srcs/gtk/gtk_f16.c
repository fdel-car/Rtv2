...
void normal_widget1(t_norm *me, char *s_entry, t_obj *current_obj)
{
	...
	me->buffer_norm[0] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_obj->norm.y);
	me->buffer_norm[1] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_obj->norm.z);
	me->buffer_norm[2] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
