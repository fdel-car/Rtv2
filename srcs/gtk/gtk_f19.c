							   ...
							   char *s_entry, t_light *current_light)
{
	...
	me->buffer_pos[0] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_light->pos.y);
	me->buffer_pos[1] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_light->pos.z);
	me->buffer_pos[2] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_light->rayon);
	me->buffer_rayon = gtk_entry_buffer_new(s_entry, strlen(s_entry));
