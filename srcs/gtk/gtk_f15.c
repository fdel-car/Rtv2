...
void direction_widget1(t_norm *me, char *s_entry, t_obj *current_obj)
{
	...
	me->buffer_dir[0] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_obj->dir.y);
	me->buffer_dir[1] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_obj->dir.z);
	me->buffer_dir[2] = gtk_entry_buffer_new(s_entry, strlen(s_entry));
...
void direction_widget2(t_norm *me, GtkWidget *grid, t_obj *current_obj)
{
...
void direction_widget(t_obj *current_obj, GtkWidget *grid, char *s_entry)
{
	...
	direction_widget1(&me, s_entry, current_obj);
	direction_widget2(&me, grid, current_obj);
