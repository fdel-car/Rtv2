...
void create_materw_object1(t_norm *me, t_obj *current_obj, char *s_entry)
{
	...
	me->shiny_buffer = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_obj->mater.int_refl);
	me->refl_buffer = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%f", current_obj->mater.int_trans);
	me->trans_buffer = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%d", current_obj->mater.color.r);
	me->color_r_buffer = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%d", current_obj->mater.color.g);
	me->color_g_buffer = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	sprintf(s_entry, "%d", current_obj->mater.color.b);
	me->color_b_buffer = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	me->text_buffer = gtk_entry_buffer_new(s_entry, strlen(s_entry));
	free(s_entry);
...
void create_materw_object2(t_norm *me, t_obj *current_obj, GdkRGBA *col)
{
	...
	me->entry_col->desc = strdup("color");
