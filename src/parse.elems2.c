...
void load_material2(t_mater *new, float *tsize, char *s)
{
	if (ft_sii(s, "brillance:"))
		new->shiny = atof(get_after(s, "brillance:"));
	if (ft_sii(s, "reflection:"))
		new->int_refl = atof(get_after(s, "reflection:"));
	if (ft_sii(s, "transparence:"))
		new->int_trans = atof(get_after(s, "transparence:"));
	if (ft_sii(s, "indice:"))
		new->indice = atof(get_after(s, "indice:"));
	if (ft_sii(s, "tsize:"))
		*tsize = atof(get_after(s, "tsize:"));
	if (ft_sii(s, "color:"))
...
void load_object2(t_obj *new, char *s)
{
	...
	if (ft_sii(s, "sphere_cut:"))
		new->sphere_cut = atof(get_after(s, "sphere_cut:"));
	if (ft_sii(s, "dir:"))
...
{
	if (ft_sii(s, "min:"))
		new->min = atof(get_after(s, "min:"));
	if (ft_sii(s, "max:"))
		new->max = atof(get_after(s, "max:"));
	if (ft_sii(s, "rayon:"))
		new->rayon = atof(get_after(s, "rayon:"));
	if (ft_sii(s, "alpha:"))
		new->alpha = atof(get_after(s, "alpha:")) * M_PI / 180;
	...
	if (ft_sii(s, "src:"))
		new->src = strdup(get_after(s, "src:"));
}
...
void load_light2(t_light *new, char *s)
{
	...
	if (ft_sii(s, "intensity:"))
		new->intensity = atof(get_after(s, "intensity:"));
	if (ft_sii(s, "blur:"))
		new->blur = atoi(get_after(s, "blur:"));
}
